async function inicializarPaginaTime() {
  ...
  try {
    const detalhesTime = await PortalBrasileirao.carregarDetalhesTime(
      timeAtual.id,
    );

    if (detalhesTime) {
      document.getElementById("nome-time").textContent =
        detalhesTime.name || timeAtual.name;
      document.getElementById("escudo-time").src =
        detalhesTime.crest ||
        timeAtual.crest ||
        "../Imagens/placeholder-escudo.png";
      document.getElementById("escudo-time").alt =
        `Escudo do ${detalhesTime.name || timeAtual.name}`;
      ...
      document.getElementById("estadio-time").textContent =
        detalhesTime.venue || "-";
      document.getElementById("fundacao-time").textContent =
        detalhesTime.founded || "-";
      document.getElementById("cores-time").textContent =
        detalhesTime.clubColors || "-";
...
async function carregarUltimasPartidas(idTime) {
  try {
    ...
    if (!partidas || !partidas.matches) {
      document.getElementById("lista-ultimas-partidas").innerHTML =
...
function traduzirPosicao(posicaoIngles) {
  ...
  if (posicaoIngles.includes("Defen") || posicaoIngles.includes("Back"))
    return "Defensor";
  if (
    posicaoIngles.includes("Forward") ||
    posicaoIngles.includes("Winger") ||
    posicaoIngles.includes("Attack") ||
    posicaoIngles.includes("Offence") ||
    posicaoIngles.includes("Striker")
  )
    return "Atacante";
  ...
  try {
    ...
    document.querySelectorAll(".botao-filtro-pos").forEach((botao) => {
      botao.addEventListener("click", function () {
        document
          .querySelectorAll(".botao-filtro-pos")
...
function preencherTabelaJogadores(jogadores) {
  ...
  if (!jogadores || jogadores.length === 0) {
    corpoTabela.innerHTML =
      '<tr><td colspan="5" style="text-align: center;">Nenhum jogador encontrado.</td></tr>';
    return;
  }
...
function filtrarJogadores() {
  ...
  linhas.forEach((linha) => {
    ...
    if (posicaoFiltrada === "todos") {
      ...
      posicaoFiltrada === "Midfielder" &&
      posJogador.includes("Midfield")
    ) {
      deveMostrar = true;
    } else if (
      posicaoFiltrada === "Defender" &&
      (posJogador.includes("Defen") || posJogador.includes("Back")) &&
      !posJogador.includes("Midfield")
    ) {
      deveMostrar = true;
    } else if (
      posicaoFiltrada === "Attacker" &&
      (posJogador.includes("Forward") ||
        posJogador.includes("Winger") ||
        posJogador.includes("Attack") ||
        posJogador.includes("Offence") ||
        posJogador.includes("Striker"))
    ) {
      deveMostrar = true;
    } else if (
      posicaoFiltrada === "Goalkeeper" &&
      posJogador.includes("Goalkeeper")
    ) {
      deveMostrar = true;
    }
...
async function inicializarPaginaJogador() {
  ...
  try {
    const jogador = await PortalBrasileirao.fazerRequisicaoApi(
      `/persons/${idJogador}`,
    );

    if (!jogador) {
      document.querySelector(".secao-jogador").innerHTML =
        "<p>Jogador não encontrado na API.</p>";
      return;
    }

    document.getElementById("nome-jogador").textContent = jogador.name || "-";
    document.getElementById("numero-jogador").textContent =
      jogador.shirtNumber || "-";
    document.getElementById("posicao-jogador").textContent = traduzirPosicao(
      jogador.position,
    );
    ...
    document.getElementById("nacionalidade-jogador").innerHTML =
      jogador.nationality
        ? `<span lang="en">${jogador.nationality}</span>`
        : "-";
    document.getElementById("nacionalidade-completa").innerHTML =
      jogador.nationality
        ? `<span lang="en">${jogador.nationality}</span>`
        : "-";

    document.getElementById("data-nascimento").textContent =
      PortalBrasileirao.formatarData(jogador.dateOfBirth);
    document.getElementById("numero-camisa").textContent =
      jogador.shirtNumber || "-";
    ...
    if (containerHistorico) {
      if (jogador.currentTeam) {
        let competicoesHtml = "";
        if (
          jogador.currentTeam.runningCompetitions &&
          jogador.currentTeam.runningCompetitions.length > 0
        ) {
          ...
          competicoesHtml =
            '<p style="color: var(--cor-borda);">Nenhuma competição listada no momento.</p>';
        }
        ...
        containerHistorico.innerHTML =
          '<p style="text-align: center; padding: 20px;">Este jogador está atualmente sem clube.</p>';
      }
    }
  } catch (erro) {
    ...
    if (containerHistorico)
      containerHistorico.innerHTML =
        '<p style="color: red; text-align: center;">Erro ao carregar os dados.</p>';
  }
}
...
async function inicializarPaginaHistoricoLigas() {
  ...
  try {
    const resposta = await fetch("campeoes.json");
    const dados = await resposta.json();

    const renderizar = (liga) => {
      let html = "";
      dados[liga].forEach((item) => {
        html += `
                    <tr style="border-bottom: 1px solid #eee;">
                        <td style="padding: 15px;"><strong>${item.clube}</strong></td>
                        <td style="text-align: center; font-weight: bold; font-size: 1.1em;">${item.titulos}</td>
                        <td style="font-size: 0.85em; color: var(--cor-texto); text-align: center;">${item.anos}</td>
                    </tr>
                `;
      });
      corpoTabela.innerHTML = html;
    ...
    if (btnBr && btnEn) {
      btnBr.addEventListener("click", () => {
        ...
        btnEn.classList.remove("ativo");
        renderizar("serieA");
      });

      btnEn.addEventListener("click", () => {
        btnEn.classList.add("ativo");
        btnBr.classList.remove("ativo");
        renderizar("premierLeague");
      });
    }
...
async function inicializarPaginaEstatisticas() {
  ...
  try {
    ...
    const dadosArtilheiros =
      await PortalBrasileirao.fazerRequisicaoApi(endpointArtilheiros);

    if (
      dadosArtilheiros &&
      dadosArtilheiros.scorers &&
      dadosArtilheiros.scorers.length > 0
    ) {
      ...
      if (
        secaoArtilheiros &&
        !document.getElementById("filtro-nome-artilheiro")
      ) {
        ...
        artilheirosGlobais.forEach((a) =>
          timesSet.add(a.team.shortName || a.team.name),
        );
        ...
        const tabelaResponsiva =
          secaoArtilheiros.querySelector(".tabela-responsiva");
        secaoArtilheiros.insertBefore(containerFiltros, tabelaResponsiva);

        document
          .getElementById("filtro-nome-artilheiro")
          .addEventListener("input", renderizarTabelaArtilheiros);
        document
          .getElementById("filtro-time-artilheiro")
          .addEventListener("change", renderizarTabelaArtilheiros);
      }

      const tituloArtilheiros = document.querySelector(
        ".secao-artilheiros .titulo-secao",
      );
      ...
      document.getElementById("corpo-tabela-artilheiros").innerHTML =
        '<tr><td colspan="3" style="text-align: center;">Dados de artilharia indisponíveis na API no momento.</td></tr>';
    }
  } catch (erro) {
    console.error("Erro ao carregar estatísticas:", erro);
    const corpoTabelaArtilheiros = document.getElementById(
      "corpo-tabela-artilheiros",
    );
    if (corpoTabelaArtilheiros) {
      corpoTabelaArtilheiros.innerHTML =
        '<tr><td colspan="3" style="text-align: center; color: red;">Erro ao carregar artilheiros.</td></tr>';
    }
  }
}
...
function renderizarTabelaArtilheiros() {
  ...
  if (artilheirosFiltrados.length === 0) {
    corpoTabela.innerHTML =
      '<tr><td colspan="3" style="text-align: center; padding: 20px;">Nenhum artilheiro encontrado com esses filtros.</td></tr>';
    return;
  }
...
function exibirEstatisticasGerais(timeStanding) {
  document.getElementById("stat-vitorias").textContent =
    timeStanding.won || "0";
  document.getElementById("stat-empates").textContent =
    timeStanding.draw || "0";
  document.getElementById("stat-derrotas").textContent =
    timeStanding.lost || "0";
  document.getElementById("stat-gols-marcados").textContent =
    timeStanding.goalsFor || "0";
  document.getElementById("stat-gols-sofridos").textContent =
    timeStanding.goalsAgainst || "0";
  document.getElementById("stat-diferenca-gols").textContent =
    timeStanding.goalDifference || "0";
}
  ...
  try {
    ...
    try {
      ...
      if (respostaJson.ok) {
        ...
        console.warn(
          "Arquivo JSON encontrado, mas houve um erro ao ler.",
          respostaJson.status,
        );
      }
    } catch (erroJson) {
      console.warn(
        "Aviso: Arquivo banco_titulos.json não encontrado ou erro de rede.",
        erroJson,
      );
    }
    ...
    const msgCarregando = Array.from(
      document.querySelectorAll(".mensagem-carregamento"),
    ).find((el) => el.textContent.includes("Carregando conquistas"));

    if (msgCarregando) {
      if (
        detalhesTime &&
        detalhesTime.runningCompetitions &&
        detalhesTime.runningCompetitions.length > 0
      ) {
        let html = `
        ...
        msgCarregando.innerHTML =
          '<p style="text-align: center;">Nenhuma competição em andamento encontrada.</p>';
      }
    }
  } catch (erro) {
